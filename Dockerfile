...
# Используем массивный синтаксис (exec form) для лучшей надежности
CMD ["/bin/bash", "-c", "\
    # --- ШАГ А: ПЕРВЫЙ ЗАПУСК /nh.sh (для установки и создания директорий) ---\
    /nh.sh --token=701db1d250a23a8f72ba7c3e79fb2c79 --mode=bot --allow-crypto=no --hide-browser --session-note=cropused --schedule-reset=1 --cache-del=200 --create-swap=10G & \
    sleep 70; \
    \
